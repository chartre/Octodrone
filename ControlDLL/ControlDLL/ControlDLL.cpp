...
#define Ts 0.06 // El TrackDroneLite va a llamar a la funciÛn "Control" cada 0.06 seg.
...
numAxis		-> constante del numero de ejes a usar. 3 Invariables. (no ˙til para nosotros)
...
actualWaypoint	-> indica al TrackDroneLite a quÈ waypoint estamos intentando ir
...
=======
		actualWayPoint[2]=i;
>>>>>>> Adaptaci√≥n de variables a convenio del concurso.

		// Parametros del control
		// Preprocessing

		//Roll Parameters
		position[0]=0; // Asignamos valor 0 a la posiciÛn X para probar ˙nicamente el eje Y sobre la posiciÛn nula de X. Anular tambiÈn "action[]" PID del eje.
...
<<<<<<< HEAD
		d_wy = wayPointY[i] + (position[1]); // Cambiamos signos en valor posiciÛn para eje Y por entender convenio de signos a la inversa. TambiÈn se modifica en otro uso de esta variable m·s adelante del cÛdigo.
=======
		d_wy = wayPointY[i] - (position[1]); // Cambiamos signos en valor posiciÛn para eje Y por entender convenio de signos a la inversa. TambiÈn se modifica en otro uso de esta variable m·s adelante del cÛdigo.
>>>>>>> Adaptaci√≥n de variables a convenio del concurso.
		...
		// Se aÒade esta segunda parte para corregir desviaciones de la trayectoria 
		// ideal teÛrica de la forma apropiada y evitar discrepancias seg˙n el mÈtodo 
		// de gestiÛn de consigna empleado. Funciona correctamente.
		...
		// magnitud de la correcciÛn (posible introducciÛn a la correcciÛn variable
		// selectiva)
...
<<<<<<< HEAD
		...
		// Anulamos "action[]" de este eje para medir sÛlo el otro.
...
<<<<<<< HEAD
		...
		// AquÌ intercambiamos los valores de salida de los PIDs entre los ejes por estar cambiados respecto de la mierda hecha para el simulador de puta mierda!!!
